...
<section class="py-20 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div id="gallery-grid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <?php $__currentLoopData = $displayedImages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $image): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="gallery-item group relative overflow-hidden rounded-2xl shadow-lg cursor-pointer h-80" onclick="openModal('<?php echo e(asset($image['path'])); ?>', '<?php echo e($image['name']); ?>')">
                    <img src="<?php echo e(asset($image['path'])); ?>" alt="<?php echo e($image['name']); ?>" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                    <div class="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        <div class="absolute bottom-0 left-0 right-0 p-6 text-white">
                            <h3 class="text-xl font-bold mb-2"><?php echo e($image['name']); ?></h3>
                            <p class="text-sm"><?php echo e(date('d M Y')); ?></p>
                        </div>
                    </div>
                </div>
        ...
        <script>
        const allImages = <?php echo json_encode($allImages, 15, 512) ?>;
        let currentIndex = 12;
        ...
        function loadMoreImages() {
            ...
            nextImages.forEach(image => {
                ...
                imageDiv.onclick = () => openModal('<?php echo e(asset('')); ?>' + image.path, image.name);
                
                imageDiv.innerHTML = `
                    <img src="<?php echo e(asset('')); ?>${image.path}" alt="${image.name}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                    <div class="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        <div class="absolute bottom-0 left-0 right-0 p-6 text-white">
